...
| `GOOGLE_SERVICE_ACCOUNT_JSON` | Google Sheets API用サービスアカウントキーJSON |
| `GOOGLE_SHEET_ID` | 管理台帳を同期するGoogle Spreadsheet ID |
| `GOOGLE_SHEET_TAB` | 同期先タブ名。未設定なら`Content Index` |

`GITHUB_TOKEN`はActionsが自動発行するため登録不要です。

## GitHub Actions権限

Slack承認後にPRを自動マージするため、GitHub側でActionsの書き込み権限を有効にします。
...
mainブランチに「必須レビュー」などのブランチ保護を設定している場合、Actionsによる自動マージが止まります。Slack承認だけを唯一の人間ゲートにする場合は、生成物PRに必須レビューを要求しない設定にします。
...
公開後のnote URLは、GitHub Actionsの`Register published note URL`を実行して登録します。このworkflowもPRを作成して自動マージします。

1. GitHubの`Actions → Register published note URL → Run workflow`を開く。
2. `issue_number`へ対象Issue番号、`note_url`へ公開済みnote URLを入力する。
3. PRが自動マージされる。
4. `admin/content-index.csv`、`memory/articles/issue-N.md`、Google Sheetsからnoteへリンクできるようになります。
...
`admin/content-index.csv`が`main`へ入るたびに、GitHub Actionsの`Sync content ledger to Google Sheets`がGoogle Sheetsへ同期します。

初回だけGoogle Cloudでサービスアカウントを作成し、同期先スプレッドシートへ編集者として共有します。

1. Google Cloud Consoleでプロジェクトを作成または選択する。
2. Google Sheets APIを有効化する。
3. サービスアカウントを作成する。
4. サービスアカウントのJSONキーを作成し、JSON全文をGitHub Secret `GOOGLE_SERVICE_ACCOUNT_JSON`へ登録する。
5. Google Sheetsで管理台帳用スプレッドシートを作成する。
6. URL中の`/d/`と`/edit`の間にあるSpreadsheet IDをGitHub Secret `GOOGLE_SHEET_ID`へ登録する。
7. スプレッドシート右上の`共有`から、サービスアカウントのメールアドレスを`編集者`として追加する。
8. 必要ならGitHub Secret `GOOGLE_SHEET_TAB`へ同期先タブ名を登録する。未設定なら`Content Index`です。

この設定後は、Slack承認だけで記事生成、GitHub反映、Google Sheets同期まで進みます。
